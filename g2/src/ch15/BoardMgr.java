...
public class BoardMgr {
	...
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			...
			pstmt.setString(10, multi.getParameter("rstatus"));
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public void updateBoard(BoardBean bean) {
		...
		try {
			con = pool.getConnection();
			sql = "update tblHelp set name=?,title=?,content=? where num=?";
			pstmt = con.prepareStatement(sql);
	...
	public void replyBoard(BoardBean bean) {
		...
		try {
			...
			sql = "insert tblHelp (name,content,title,ref,pos,depth,regdate,pass,count,ip)";
			sql += "values(?,?,?,?,?,?,now(),?,0,?)";
	...
	public void post1000(){
		...
		try {
			con = pool.getConnection();
			sql = "insert tblHelp(name,content,subject,ref,pos,depth,regdate,pass,count,ip,filename,filesize)";
